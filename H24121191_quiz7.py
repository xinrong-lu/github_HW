...
def list_books_by_genre():
    ...
    # your code here
      genre_books = [(title, info['price']) for title, info in library.items() if info['genre'] == genre]
    if genre_books:
        sorted_books = sorted(genre_books, key=lambda x: x[0])
        for title, price in sorted_books:
            print(f"Title: {title}\nPrice: {price}")
    else:
